...
def extract_plain_text_outside_tables(pdf_path: str, output_json: str, y_tol: float = 5.0) -> dict:
    """
    Extract ONLY plain text that is OUTSIDE detected table bounding boxes.
    - If a page has no table bboxes, the whole page text is returned.
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page_number, page in enumerate(pdf.pages, start=1):
            # 1) detect table bounding boxes on the page
            ...
            for tbl in page.find_tables():
                ...
                try:
                    ...
                    # ignore tables that don't expose bbox
                    continue

            # 2) if there are NO tables, return entire page text (if any)
            if not table_bboxes:
            ...
            # 3) extract words and filter out those that fall inside any table bbox
            words = page.extract_words()  # words have x0,x1,top,bottom,text
            ...
            for w in words:
                try:
                    ...
                    # if coordinates missing skip the word
                    continue
            ...
            # 4) if there are outside words, merge them into readable lines
            if outside_words:
            ...
            # if no outside words for this page -> skip page (user requested "if there is no give no result")

    result = {"plain_text": pages_out} if pages_out else {}

    # write JSON (will be empty object {} if nothing found)
